...
""",
    ...
    parser.add_argument(
        ...
        dest="hide_api_keys",
        action=argparse.BooleanOptionalAction,
        help="Mask API keys in logs",
    )
    parser.add_argument(
        "--langchain-tracing",
        dest="langchain_tracing",
        action=argparse.BooleanOptionalAction,
        help="Enable LangSmith tracing (LANGCHAIN_TRACING_V2)",
    )
    parser.add_argument("--langchain-project", type=str, help="LangSmith project name")
...
class Config:
    ...
    def from_args(cls, args: argparse.Namespace) -> "Config":
        ...
        if args.langchain_tracing is not None:
            config.langchain_tracing_v2 = args.langchain_tracing
        if args.langchain_project is not None:
            config.langchain_project = args.langchain_project
    ...
    def from_env(cls) -> "Config":
        ...
        - REASONING_EFFORT: Enable reasoning (none/low/medium/high)
        ...
        if env_val := os.getenv("REASONING_EFFORT"):
            config.reasoning_effort = env_val.lower()
        else:
            config.reasoning_effort = "none"  # default value
        ...
        # LangSmith observability
        if env_val := os.getenv("LANGCHAIN_TRACING_V2"):
            config.langchain_tracing_v2 = env_val.lower() == "true"
        if env_val := os.getenv("LANGCHAIN_PROJECT"):
            config.langchain_project = env_val
        if env_val := os.getenv("LANGCHAIN_API_KEY"):
            config.langchain_api_key = env_val
        elif os.getenv("LANGCHAIN_TRACING_V2"):
            config.langchain_api_key = os.getenv("ANONYMIZED_TELEMETRY", "")
    ...
    def validate(self) -> List[str]:
        ...
        if self.provider not in ("openai", "ollama", "anthropic", "google"):
            errors.append(
                f"Invalid provider: {self.provider}. Must be 'openai', 'ollama', 'google', or 'anthropic'"
            )
        ...
        if self.reasoning_effort not in ("none", "low", "medium", "high", None):
            errors.append(
                f"Reasoning effort must be 'none', 'low', 'medium', or 'high', got {self.reasoning_effort}"
            )
    ...
    def __post_init__(self):
        ...
        # Set default reasoning_effort if not provided
