...
"""
    ...
    parser.add_argument("-p", "--provider", type=str, choices=["openai", "ollama", "anthropic", "google"], help="LLM provider")
...
class Config:
    ...
    def validate(self) -> List[str]:
        ...
        if self.provider not in ("openai", "ollama", "anthropic", "google"):
            errors.append(f"Invalid provider: {self.provider}. Must be 'openai', 'ollama', 'google', or 'anthropic'")
        ...
        if self.reasoning_effort not in ("none", "low", "medium", "high", None):
            errors.append(f"Reasoning effort must be 'none', 'low', 'medium', or 'high', got {self.reasoning_effort}")
